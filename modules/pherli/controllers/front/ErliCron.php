...
//                                $return['body'] = self::cleanBody($return['body']);
...
//                                $return['body'] = 'Błąd E#'.$return['status'].' ['.$body->failureType.'] '.$body->payload;
...
//                    $return['body'] = $return['info'];
                    ...
                    $bodys = json_decode($return['body'], true);
                    $details = $bodys['payload']['details'];
                    $detail = '';
                    foreach ($details as $keyI => $item) {
                        $detail .= '<br />'.$keyI.': '.$item;
                    }
                    $return['body'] = $return['body'];//$return['info'].': '.$detail;
                } elseif ($return['status'] == 429) {
//                    $return['body'] = 'Nieoczekiwany błąd';
                } else*/
                if ($return['status'] == 400){
//                    $bodys = json_decode($return['body'], true);
//                    $details = $bodys['payload']['details'];
...
//                    $return['body'] = $return['info'];
                    ...
                    if ($this->debug) {
                        echo '<br />DETAIL: '.$detail;//exit();
                    }
                    $return['body'] = $return['info'].': '.$detail;*/
                } elseif ($return['status'] == 429) {
//                    $return['body'] = 'Nieoczekiwany błąd';
                } else {
                    $body = $return['body'];
...
//                        Erli::setDisableProductInErli((int)$id_product);
                        ...
                        ErliSync::updateProductSync((int)$id_product, (int)$id_product_attribute, (int)$id_sync, $return['status']);
//                        Erli::updateInErliProduct((int)$id_product, 1);
                    }
                }
                Erli::updateStatus($id_product, $status);
            } else {
...
//                        Erli::updateInErliProduct((int)$id_product, 1);
    ...
    {
//        $products = Erli::getProductByErliExistActive(0, 1);
