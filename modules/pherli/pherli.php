...
require_once __DIR__.'/classes/AuthDSErli.php';
...
class pherli extends Module
{
    ...
    private function isActiveModule()
    {
        ...
        $auth = new AuthDSErli($this->name, $shop->getBaseURL());
    ...
    private function prestahelp()
    {
        ...
        $banersHtml = AuthDSErli::getBanersHtml();
...
//                        'vendors' => $vendors,
            ...
            if (Tools::getIsset('sendMailInfo')) {
                $subject = 'Zgłoszenie o braku licencji';
                $email = 'rafal@egzyl.pl';
                ...
                $template_vars = array(
                    ...
                    '{shopEmail}' => $configuration['PS_SHOP_EMAIL'],
                    '{shopName}' => $configuration['PS_SHOP_NAME'],
                );
                $iso = Language::getIsoById($id_lang);
                if (file_exists(dirname(__FILE__).'/mails/'.$iso.'/'.$template.'.txt')
                    && file_exists(dirname(__FILE__).'/mails/'.$iso.'/'.$template.'.html')) {
                    Mail::Send($id_lang, $template, $subject, $template_vars, $email, null, $configuration['PS_SHOP_EMAIL'], $configuration['PS_SHOP_NAME'], null, null, dirname(__FILE__).'/mails/');
                    Configuration::updateValue('PHDESCRIPTION_SEND_MAIL', date('Y-m-d H:i:s'));
                }
                Tools::redirectLink($_SERVER['HTTP_REFERER']);
            }
            $show_send = true;
            $date = Configuration::get('ERLI_SEND_MAIL');
            if (!empty($date)) {
...
//                    Erli::updateDeliveryVendor((int)$key, $vendor);
//                }
//            }
            Tools::redirect($_SERVER['HTTP_REFERER']);
        }
...
//        $update1013_sql = Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'hook` WHERE `name` = "actionProductSaveBefore"');
        $update1013a_sql = Db::getInstance()->executeS('SHOW COLUMNS FROM `' . _DB_PREFIX_ . 'erli_products` WHERE `Field` = "images"');
        if (empty($update1013a_sql)) {
            $returned = true;
        }
...
//                    $data['deliveryTracking']['vendor'] = $deliveryErli['typeId'];
...
//                    print_r($paymentInfo);exit();
    ...
    public function setProductToErli($id_product, $id_product_attribiute = 0, $deliveryTime, $deliveryPrice, $active)
    {
        ...
        $images = $product->getImages((int)$this->context->cookie->id_lang);
        if (!empty($images)) {
//            $send['images'][0]['url'] = $this->context->link->getImageLink($product->link_rewrite, (int)$images[0]['id_image']);
...
//                    $send['importantFeatures'][$key] = $feature_erli;
                }
            }
        }
