...
// Ścieżka do pliku: /modules/ps_facetedsearch/src/Filters/Block.php
...
class Block
{
    ...
    public function getFromCache($filterHash)
    {
        ...
        if (!empty($row)) {
            ...
            // Błąd "unserialize(): Error at offset" występuje, gdy dane w bazie są obcięte (kolumna TEXT 64KB).
            // Używamy @, aby stłumić błąd (Notice), ponieważ wiemy, że może on wystąpić.
            
            $serializedData = current($row);
            $data = @unserialize($serializedData);

            // Sprawdzamy, czy deserializacja się nie powiodła.
            // Musimy również sprawdzić, czy oryginalna wartość to nie było przypadkiem 'b:0;' (serialize(false)),
            // co jest poprawną wartością, ale @unserialize zwróci false.
            if ($data === false && $serializedData !== serialize(false)) {
                // Błąd podczas deserializacji, prawdopodobnie obcięte dane.
                // Zwracamy null, aby wymusić ponowne wygenerowanie cache.
                return null;
            }

            // Zwracamy poprawnie zdeserializowane dane
            return $data;
            // *** KONIEC MODYFIKACJI ***
        }
