/**
 * /modules/dietamamyto/views/js/dmto_step2_progress.js
 */
(function() {
  function $(sel, ctx){return (ctx||document).querySelector(sel);}
  function setProgress(pct){
    var bar = $('#dmto-step2-bar');
    if(!bar) return;
    bar.style.width = pct+'%';
    bar.textContent = pct+'%';
  }
  function setStatus(txt){
    var el = $('#dmto-step2-status');
    if(el) el.textContent = txt || '';
  }
  function disableForm(disabled){
    var form = $('#dmto-step2-form');
    if(!form) return;
  ...
  function startBatch(){
  var box = document.getElementById('dmto-step2-progress');
  if (box) {
    box.style.display = 'block';
  }

  var detailsEl = document.getElementById('dmto-step2-details');
  var errorsEl  = document.getElementById('dmto-step2-errors');
  if (detailsEl) {
    detailsEl.innerHTML = '';
  }
  if (errorsEl) {
  ...
  var form = document.getElementById('dmto-step2-form');
  var fd;
  ...
  fd.append('ajax', '1');
  fd.append('action', 'Step2Start');
  if (typeof DMTO_STEP2 !== 'undefined' && DMTO_STEP2.token) {
    fd.append('token', DMTO_STEP2.token);
  }

  fetch(DMTO_STEP2.ajax, {
    method: 'POST',
    ...
    .then(function (json) {
      ...
      setStatus('Przygotowano ' + (json.total || 0) + ' produktów...');
      loop(json.job, 0, 0, [], {});
    })
    .catch(function (err) {
  ...
  function loop(job, totalUpdated, totalCreated, createdNames, failedProducts){
    var url = DMTO_STEP2.ajax + '&ajax=1&action=Step2Chunk&job=' + encodeURIComponent(job) + '&token=' + encodeURIComponent(DMTO_STEP2.token);
    fetch(url, {credentials:'same-origin'})
      ...
      .then(json => {
        ...
        if(json.finished){
          ...
          var detailsEl = $('#dmto-step2-details');
          ...
          var errorsEl = $('#dmto-step2-errors');
          ...
          if (DMTO_STEP2.cleanupEnabled) {
              runCleanup(finalMessage);
          } else {
              disableForm(false);
              var bar = $('#dmto-step2-bar'); if(bar){ bar.classList.remove('active'); }
          }
          return;
        }
  ...
  function runCleanup(previousStatus) {
    ...
    var cleanupUrl = DMTO_STEP2.ajax + '&ajax=1&action=CleanupUnusedValues&token=' + encodeURIComponent(DMTO_STEP2.token);
  ...
  document.addEventListener('DOMContentLoaded', function(){
    var btn = document.getElementById('dmto-step2-start-batch');
