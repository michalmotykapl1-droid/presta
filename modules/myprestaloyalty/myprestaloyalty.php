    ...
    public function hookactionOrderReturn($params)
    {
        ...
        foreach ($details as $detail) {
            //if ($taxesEnabled == PS_TAX_EXC) {
                $unit_price_tax_incl = Db::getInstance()->getValue('SELECT ROUND(unit_price_tax_incl, 2)
				FROM ' . _DB_PREFIX_ . 'order_detail od
				WHERE id_order_detail = ' . (int)$detail['id_order_detail']);
                $total_price += $unit_price_tax_incl * $detail['product_quantity'];
            /**
            } else {
                $unit_price_tax_excl = Db::getInstance()->getValue('SELECT ROUND(unit_price_tax_excl, 2)
				FROM ' . _DB_PREFIX_ . 'order_detail od
				WHERE id_order_detail = ' . (int)$detail['id_order_detail']);
                $total_price += $unit_price_tax_excl * $detail['product_quantity'];
            }
             **/
        }
