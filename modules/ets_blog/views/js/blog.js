...
$(document).ready(function(){
    ...
    // --- POCZĄTEK KODU DO ROZWIJANIA ŹRÓDEŁ ---
    // Ukryj treść źródeł na starcie, jeśli istnieje
    if ($('#sourcesContent').length) {
        $('#sourcesContent').hide();
    }

    // Dodaj obsługę kliknięcia dla nagłówka źródeł
    $(document).on('click', '#sourcesToggle', function() {
        // Rozwiń/zwiń kontener z treścią z animacją
        $('#sourcesContent').slideToggle(300);
        // Zmień wygląd nagłówka (np. obrót strzałki), dodając klasę 'active'
        $(this).toggleClass('active');
    });
    // --- KONIEC KODU ---
});
...
var ets_blog = {
    ...
    /* --- USUNIĘTO KOD KARUZELI DLA NAJNOWSZYCH WPISÓW ---
     if($('.ets_blog_block_latest ul').length > 0 && typeof $.fn.etsowlCarousel == 'function')
     {
        $('.ets_blog_block_latest ul').etsowlCarousel({
            items : 1,
            nav : true,  
            navigation : true,
            navigationText : ["",""],
            pagination : false,
            loop: $(".ets_blog_block_latest ul li").length > 1,
            rewindNav : false,
            dots : false,        
            navText: ['', ''],  
            callbacks: true,
