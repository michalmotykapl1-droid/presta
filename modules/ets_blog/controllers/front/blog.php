...
class Ets_blogBlogModuleFrontController extends ModuleFrontController
{
    ...
    public function displayListPosts(){
        ...
        if (($search = urldecode(trim(Tools::getValue('search'))))!='' && Validate::isCleanHtml($search))
        {
            // OSTATECZNA POPRAWKA - Zmiana na standardowe kodowanie utf8_general_ci
            $search_filter = " AND (pl.title LIKE '%".pSQL($search)."%' OR pl.description LIKE '%".pSQL($search)."%' OR pl.short_description LIKE '%".pSQL($search)."%')";
            $params['search'] = $search;

            $all_matching_posts = Ets_blog_post::getPostsWithFilter($base_filter . $search_filter, $sort, 0, 9999);
            if ($all_matching_posts) {
                $found_categories_with_count = array();
                foreach ($all_matching_posts as $post) {
                    $post_categories = Ets_blog_post::getCategoriesByIdPost($post['id_post'], false, true);
                    if (!empty($post_categories)) {
                        foreach($post_categories as $cat) {
                            if (!isset($found_categories_with_count[$cat['id_category']])) {
                                $search_params = array('search' => urlencode($search), 'filter_id_category' => $cat['id_category']);
                                $cat['link'] = $this->module->getLink('blog', $search_params);
                                $cat['post_count'] = 1;
                                $found_categories_with_count[$cat['id_category']] = $cat;
                            } else {
                                $found_categories_with_count[$cat['id_category']]['post_count']++;
                            }
                        }
                    }
                }
                $this->context->smarty->assign('blog_found_categories', $found_categories_with_count);
            }
            
            $filter .= $search_filter;

            if (($filter_id_category = (int)Tools::getValue('filter_id_category'))) {
                $filter .= " AND pc.id_category = " . (int)$filter_id_category;
                $params['filter_id_category'] = $filter_id_category;
            }
        }
